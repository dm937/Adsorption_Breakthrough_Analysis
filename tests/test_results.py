...
class TestVersion(unittest.TestCase):
    ...
    def _load_pybreak_obj(self,ExperimentalSetup,root_path,blank):
        ...
        if blank == True:
            ...
            coriolis_file_name = os.path.join(root_path,"220819-ZIF-8-dry-22%and60%40C20220819134938.txt")
            MS_file_name = os.path.join(root_path,"220819ZIF8Blank22and60%CO2at40C.csv")
            sorted_csv = pd.read_csv(os.path.join(save_path,"bench_220819-ZIF-8-dry-22%and60%40C20220819134938.txt_220819ZIF8Blank22and60%CO2at40C.csv.csv"),index_col = 0)
            ...
            sorted_csv = pd.read_csv(os.path.join(save_path,"bench_220824-ZIF-8-4and22%at40C.txt_220824-ZIF-8-4and22%at40C.csv.csv"),index_col = 0)
    ...
    def test_sorted_data_blank(self):
        ...
        blank = True
        pybreak_obj, loading, sorted_csv = self._load_pybreak_obj(ExperimentalSetup,root_path,blank)
        
        for key,value in pybreak_obj.calculate_loading()[0][1].items():
            self.assertAlmostEqual(pybreak_obj.calculate_loading()[0][1][key],loading[0][1][key],places=1)

        for key,value in pybreak_obj.calculate_loading()[1][1].items():
            self.assertAlmostEqual(pybreak_obj.calculate_loading()[1][1][key],loading[1][1][key],places=1)
    

        
        bench_sorted_dict = sorted_csv.describe().T[['mean','count','std']].to_dict()
        sorted_dict_compare = pybreak_obj.sorted_data.describe().T[['mean','count','std']].to_dict()
        
        for key,value in bench_sorted_dict['mean'].items():
            self.assertAlmostEqual(sorted_dict_compare['mean'][key],bench_sorted_dict['mean'][key],places=1)
            self.assertEqual(sorted_dict_compare['count'][key],bench_sorted_dict['count'][key])
            self.assertAlmostEqual(sorted_dict_compare['std'][key],bench_sorted_dict['std'][key],places=1)
